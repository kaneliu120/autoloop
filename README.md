...
将 `.claude/` 目录下的 skill 文件复制到目标项目，或在项目根目录创建符号链接：

```bash
# 在目标项目中注册 AutoLoop skill
ln -s /Users/kane/Projects/autoloop/.claude /path/to/your/project/.claude
```
...
cp -r /Users/kane/Projects/autoloop ~/.claude/skills/autoloop

# 或注册到 Claude Code config
claude skill install /Users/kane/Projects/autoloop
```
...
```
读取 /Users/kane/Projects/autoloop/SKILL.md，然后按照 AutoLoop 规范执行以下任务：{你的任务}
```
...
├── autoloop-results.tsv      # 结构化数据（T2/T4 适用）
