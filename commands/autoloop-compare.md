---
name: autoloop-compare
description: >
  AutoLoop T2: 多方案对比模板。多维度评分 + 证据支撑 + 置信度声明 + 明确推荐。
  每个选项独立分析，统一评分维度，敏感性检验，输出决策矩阵。
  质量门禁阈值见 protocols/quality-gates.md T2 行。
  触发：/autoloop:compare 或任何需要在多个选项中做决策的场景。
---

# AutoLoop T2: Compare — 多方案对比

## 执行前提

读取 `autoloop-plan.md` 获取：
- 要比较的选项列表（必须 ≥ 2 个）
- 评估维度列表（可自动生成）
- 权重配置（如果用户指定了优先级）
- key_assumptions（结构化列表，格式：假设名称 + 当前值 + 单位）

**Round 2+ OBSERVE 起点**：先读取 `autoloop-findings.md` 反思章节，获取遗留问题、有效/无效策略、已识别模式、经验教训，再扫描当前状态。详见 `protocols/loop-protocol.md` OBSERVE Step 0 章节。

---

## 默认评估维度

如果用户没有指定维度，根据选项类型自动生成。

> 以下为非规范性示例，实际维度和权重应根据对比主题动态生成。

### 技术栈/工具选型

| 维度 | 权重 | 评分说明 |
|------|------|---------|
| 功能匹配度 | 25% | 与需求的匹配程度 |
| 技术成熟度 | 20% | 版本稳定性、生产验证 |
| 学习曲线 | 15% | 上手难度 |
| 社区生态 | 15% | 文档、插件、社区活跃度 |
| 成本 | 15% | 总拥有成本（TCO） |
| 长期风险 | 10% | 维护风险、厂商锁定 |

### 架构方案对比

| 维度 | 权重 | 评分说明 |
|------|------|---------|
| 解耦程度 | 20% | 模块独立性 |
| 可扩展性 | 20% | 未来扩展的容易程度 |
| 性能 | 20% | 延迟、吞吐量 |
| 实施复杂度 | 15% | 开发和部署难度 |
| 运维成本 | 15% | 监控、维护难度 |
| 迁移风险 | 10% | 从现有架构迁移的风险 |

### 商业方案对比

| 维度 | 权重 | 评分说明 |
|------|------|---------|
| ROI 预期 | 25% | 投资回报 |
| 实施周期 | 20% | 多久能看到结果 |
| 资源需求 | 20% | 人力/资金需求 |
| 市场风险 | 20% | 外部风险 |
| 战略契合度 | 15% | 与长期目标的一致性 |

---

## 第一轮：并行选项分析

### OBSERVE（第1轮基线采集）

第1轮无历史数据，执行基线采集：当前发现数 = 0，所有质量门禁得分 = 0。
将此作为 iteration 0 基线写入 `autoloop-progress.md`。详见 `protocols/loop-protocol.md` 第1轮 Bootstrap 规则。

### 1.1 并行选项分析

#### 派遣

角色：researcher xN，每个候选方案分配 2 个 researcher 并行分析（使用不同角度：正向分析 / 批判性分析）（职责定义见 `protocols/agent-dispatch.md`）

这确保偏见检查门禁可达：每选项 ≥ 2 个独立评估。偏见检查计算方法见 `protocols/quality-gates.md` T2 偏见检查章节。

#### 本次范围

- 对比主题：{主题}
- 选项列表：{各选项名称}
- 评估维度：{维度列表}

#### 执行流程

1. 每个 researcher 对负责的选项在每个评估维度上深度分析
2. 每个维度必须有具体证据支撑（数据、引用、案例）
3. 识别核心优势（最多 3 个）、核心劣势（最多 3 个）、适用/不适用场景
4. 按角色定义的输出格式交付：维度评分表 + 优劣势 + 适用场景 + 综合得分 + 置信度

### 1.2 偏见检查（所有 researcher 返回后立即执行）

偏见检查计算方法见 `protocols/quality-gates.md` T2 偏见检查章节。

简要流程：对每个选项比较两个 researcher 的评分差，超过阈值则触发第 3 个独立 researcher 重新评估该选项，以多数结论为准。

### 1.3 独立中立审查

#### 派遣

角色：analyst（职责定义见 `protocols/agent-dispatch.md`，T2 兼任偏见检查 + 敏感性分析）

#### 执行流程

1. 读取所有选项的分析结果
2. 检查评分异常、证据质量均衡性、选择性引用、评分标准一致性
3. 输出偏见风险评估（低/中/高）+ 需重新评估的维度 + 建议增补的对立证据

---

## 第二轮：加权评分 + 敏感性分析

### 2.1 加权计算

```text
综合得分 = Σ (维度得分 × 维度权重)
```

用户配置的权重优先，默认权重次之。

计算每个选项的加权得分，生成排名。

### 2.2 敏感性分析

敏感性分析从 `autoloop-plan.md` 的 `key_assumptions` 字段读取关键假设列表。plan 中未提供则自动从评估维度中识别成本/时间/规模类维度作为假设来源。完整计算方法和通过标准见 `protocols/quality-gates.md` T2 敏感性分析章节。

输出格式：

```text
对每个关键假设正反向变动测试 → 重新推导排名 → 与原排名对比

结论：
- 推荐稳健性：{高（所有场景推荐同一选项）/ 中 / 低}
- 临界假设：{对结果影响最大的假设}
```

---

## 对比矩阵生成

```markdown
## 综合对比矩阵

| 维度（权重） | {选项 A} | {选项 B} | {选项 C} |
|------------|---------|---------|---------|
| 功能匹配度 (25%) | 8/10 | 7/10 | 6/10 |
| **加权总分** | **7.5** | **7.3** | **6.7** |
| **排名** | **第 1** | **第 2** | **第 3** |
```

---

## 推荐输出格式

```markdown
## 推荐结论

### 主推荐：{选项 A}
置信度：{高/中/低}（{基于 N 个来源}）

**推荐理由**：
{简要说明核心维度排名和分差}

主要优势：
1. {优势 1}（数据支持：{来源}）
2. {优势 2}
3. {优势 3}

主要风险：
1. {风险 1}（缓解建议：{措施}）
2. {风险 2}

### 适用条件

✓ **选择 {选项 A} 当**：{条件}
✓ **选择 {选项 B} 当**：{条件}

### 分差分析

{分差是否稳健，哪个假设变动会导致推荐反转}

### 不推荐的理由

{选项 C}：{具体原因}

---

## 证据局限性

以下信息未能充分验证，可能影响结论：
- {信息缺口 1}（影响维度：{维度}，影响程度：中）

建议在正式决策前验证：{具体验证建议}
```

---

## 结果文件

生成 `autoloop-results.tsv`（TSV 格式见 `protocols/loop-protocol.md` 统一 TSV Schema 章节）。

证据来源 URL 和详细分析记录到 `autoloop-findings.md`，不放在 results.tsv。

---

## 质量门禁检查

完整门禁定义和通过标准见 `protocols/quality-gates.md` T2 行。在输出最终推荐前，验证：

- [ ] 覆盖率达标（见 protocols/quality-gates.md T2 覆盖率门禁）
- [ ] 可信度达标（见 protocols/quality-gates.md T2 可信度门禁）
- [ ] 偏见检查已通过（计算方法见 protocols/quality-gates.md T2 偏见检查章节）
- [ ] 敏感性分析已完成（计算方法见 protocols/quality-gates.md T2 敏感性分析章节）
- [ ] 置信度已明确声明
- [ ] 信息缺口已列出

未通过则进行第三轮补充研究。

---

## 每轮 REFLECT 执行规范

每轮（包括第一轮）结束后，在 EVOLVE/终止判断之后执行。REFLECT 必须写入文件，不能只在思考中完成（规范见 `protocols/loop-protocol.md` REFLECT 章节）：

写入 `autoloop-findings.md` 的4层反思结构表（问题登记/策略复盘/模式识别/经验教训），格式见 `templates/findings-template.md`：

- **问题登记**：记录本轮发现的信息空白、来源冲突、数据质量问题、选项分析缺陷
- **策略复盘**：搜索策略/分析方法/偏见检查方式的效果评估（保持 | 避免 | 待验证）（策略评价枚举见 protocols/loop-protocol.md 统一状态枚举）
- **模式识别**：哪些维度反复缺少证据、哪些选项总是数据质量低
- **经验教训**：选项分析/偏见检查/敏感性分析的有效性总结
