...
技术栈适配：
所有技术栈使用 `{syntax_check_cmd}`（见 plan，变量名见 `protocols/loop-protocol.md` 统一参数词汇表），按 `syntax_check_file_arg` 决定是否附加文件参数。新路由在 `{main_entry_file}` 中注册（Python: include_router；Node.js: app.use/router；其他: 按项目规范）。
...
3. 新路由功能验证（如果服务器在运行）
   curl -X {方法} {URL} -H "{auth_header}: {测试Key}"（认证方式和请求头由项目决定）
   期望：HTTP 2xx，响应格式正确
...
4. Health check
   ...
   期望：HTTP 200，响应格式由项目决定（认证方式和健康检查响应格式由项目决定）
