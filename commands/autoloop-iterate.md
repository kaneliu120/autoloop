...
| 模糊描述 | 量化 KPI（示例格式，具体数值由用户在 plan 中定义）|
|---------|---------|
| "更快" | API P95 延迟 < {N}ms（范围：100-2000）|
| "更好的质量" | 测试覆盖率 ≥ {N}%（范围：60-100）+ py_compile 零错误 |
| "减少错误" | 异常日志数量下降 > {N}%（范围：50-99）|
| "代码更清晰" | 函数平均行数 < {N}（范围：20-50）+ 无 TODO 注释 |
| "更好的用户体验" | 首屏时间 < {N}s（范围：1-5）+ Lighthouse 分数 ≥ {N}（范围：60-100）|
...
- **策略复盘**：本轮改进策略的效果评估 — 实际改进量 vs 预期，保持 | 避免 | 待验证（策略评价枚举见 references/loop-data-schema.md 统一状态枚举）
