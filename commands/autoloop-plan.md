...
质量门禁阈值见 `references/quality-gates.md` 门禁评估矩阵（各模板对应行）。
默认轮次见 `references/parameters.md` 迭代控制参数章节。
...
4. 关键假设（key_assumptions）：
   ...
   用于后续敏感性分析（计算方法见 references/quality-gates.md T2敏感性分析章节）：
...
```
质量门禁（满足全部才算完成，完整定义见 references/quality-gates.md）：
...
收集所有参数后，生成完整的 `autoloop-plan.md`，必须严格遵循 `assets/plan-template.md` 的结构，包含所有章节（含 **扩展维度** 和 **策略历史**）：
...
{按 assets/plan-template.md 中该模板的完整字段填写，不得省略任何字段}

T2 的 key_assumptions 必须以结构化列表记录，格式：
  - 假设名称：{名称}，当前值：{数值}，单位：{单位}
（敏感性分析计算方法见 references/quality-gates.md T2敏感性分析章节）
...
质量门禁阈值见 `references/quality-gates.md` 门禁评估矩阵。
...
**全部达标条件**：所有维度同时达到目标阈值（见 references/quality-gates.md）
...
输出文件命名见 `references/loop-protocol.md` 统一输出文件命名章节。
...
摘要：
  ...
  目标：{一句话}
  轮次预算：{N} 轮
  质量门禁：{维度数} 个维度（完整定义见 references/quality-gates.md）
