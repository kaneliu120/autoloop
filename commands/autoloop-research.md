...
CHECK阶段由独立的 research-evaluator subagent 执行评分（调度方式见 protocols/agent-dispatch.md 独立评分器章节）。research-evaluator 只接收 findings.md 产出物，不接收执行过程信息，按 quality-gates.md 锚点盲评。
