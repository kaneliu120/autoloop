...
Agent(
  ...
  prompt='你是安全审查专家，只关注安全维度（SQL注入/命令注入/XSS/路径穿越/敏感数据暴露）。
          评分规则见 protocols/enterprise-standard.md 安全性维度，门禁阈值见 protocols/quality-gates.md 门禁评估矩阵（P1/P2/P3）。
          忽略可靠性和可维护性维度。'
)
...
Agent(
  ...
  prompt='你是可靠性审查专家，只关注可靠性维度（静默失败/缺少异常处理/无超时配置/缺少降级回退）。
          评分规则见 protocols/enterprise-standard.md 可靠性维度，门禁阈值见 protocols/quality-gates.md 门禁评估矩阵。
          忽略安全性和可维护性维度。'
)
...
Agent(
  ...
  prompt='你是可维护性审查专家，只关注可维护性维度（路由注册/模块导出/类型规范/代码重复）。
          评分规则见 protocols/enterprise-standard.md 可维护性维度，门禁阈值见 protocols/quality-gates.md 门禁评估矩阵。
          忽略安全性和可靠性维度。'
)
```
...
任务：
1. 读取以下关键文件，理解现有架构：
   - {main_entry_file}（来自 plan，见 protocols/loop-protocol.md 统一参数词汇表）
   - {相关模块文件路径}
...
代码库路径：{codebase_path}
迁移验证命令：{migration_check_cmd}（来自 plan，见 protocols/loop-protocol.md 统一参数词汇表；迁移目录和配置文件在 plan 中定义）
...
| Subagent 代码验证失败 | 返回详细错误给 subagent，要求修复（遵守统一重试上限 = 2 次，见 protocols/loop-protocol.md 统一重试规则） |
...
| `{syntax_check_cmd}` | `npx tsc --noEmit`（裸命令；在 `{frontend_dir}` 目录下执行，`{frontend_dir}` 见 protocols/loop-protocol.md 统一参数词汇表）|
...
| `{syntax_check_cmd}` | 从 plan.syntax_check_cmd 填入（见 protocols/loop-protocol.md 统一参数词汇表）|
| `{main_entry_file}` | 从 plan.main_entry_file 填入（见 protocols/loop-protocol.md 统一参数词汇表）|
| `{migration_check_cmd}` | 从 plan.migration_check_cmd 填入（见 protocols/loop-protocol.md 统一参数词汇表），如无数据库则 N/A |
