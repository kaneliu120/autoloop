...
| 测试覆盖率 < 60%（关键路径）| -1 | P2 | 关键路径无单元/集成测试 |
| 测试覆盖率 < 40%（关键路径）| -2 | P2 | 累计扣分（替代 < 60% 档）|
...
| 高频查询字段无索引（EXPLAIN 验证）| -1 | 关键查询无 EXPLAIN 分析或 EXPLAIN 显示全表扫描 |
...
| 无告警配置（关键操作）| -1 | 关键操作无监控告警，故障无法及时发现 |
| 数据库连接池无 pool_pre_ping | -0.5 | `create_async_engine()` 未设置 `pool_pre_ping=True` |
