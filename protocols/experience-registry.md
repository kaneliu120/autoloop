...
当组合策略成功时，需要通过消融测试确认每个组成部分的贡献：

```text
触发条件：组合策略 C{NN} 连续 2 次 delta > 0
消融流程：
  1. 在下一个同类任务中，依次移除组合中的一个基础策略
  2. 每次移除后执行一轮，记录 delta
  3. 对比：完整组合 delta vs 移除后 delta
  4. 如果移除某策略后 delta 无显著变化（< 阈值的 20%）→ 该策略对组合贡献低
  5. 如果移除后 delta 显著下降 → 该策略是关键组成部分

结果处理：
  - 所有基础策略都有贡献 → 组合策略保持，各基础策略的"组合贡献度"字段更新
  - 某基础策略无贡献 → 从组合中移除，简化组合策略
  - 仅一个基础策略有贡献 → 解散组合，该基础策略直接升格
```

**消融不在常规迭代中执行**，仅在以下条件满足时触发：
- 组合策略已验证有效（连续 2 次 delta > 0）
- 当前任务有足够预算（剩余 ≥ 50%）
- 用户未要求跳过消融
