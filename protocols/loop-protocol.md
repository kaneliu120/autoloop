...
**暂停等待确认状态**：当需要人工确认时（T5 Phase 0.5 文档确认、T5 Phase 5 线上验收、以及任何 blocking gate），状态机进入暂停等待确认。系统在此状态停止自动推进，等待用户输入。用户输入 `confirmed`（Phase 0.5）或 `verified`（Phase 5）后，恢复到下一阶段。用户提出修改意见时，先处理修改，再自动恢复。
