...
**任务状态（Task Status）**：
```
准备开始 | 执行中 | 暂停等待确认 | 暂停等待输入 | 已完成
```
- 准备开始 — 计划已创建，尚未执行
- 执行中 — 正在执行迭代轮次
- 暂停等待确认 — 等待用户审阅确认（如方案文档确认）
- 暂停等待输入 — 等待用户提供缺失信息（如KPI定义）
- 已完成 — 任务结束（具体原因见终止原因）
...
**AWAIT_USER 状态**：当需要人工确认时（T5 Phase 0.5 文档确认、T5 Phase 5 线上验收、以及任何 blocking gate），状态机进入 AWAIT_USER，任务状态变为"暂停等待确认"或"暂停等待输入"（视具体场景）。系统在此状态停止自动推进，等待用户输入。用户输入 `confirmed`（Phase 0.5）或 `verified`（Phase 5）后，任务状态恢复为"执行中"，进入下一阶段。用户提出修改意见时，先处理修改，再自动恢复。注意：暂停状态下任务仍然活着，不写"已完成"；只有真正结束时才写"已完成+终止原因"。
...
| 发现紧急 P1 问题（数据丢失/安全漏洞）| 立即提升，暂停其他工作（见 protocols/evolution-rules.md 进化类型 4）|
...
1. **质量门禁全部达标** → 达标终止 → TERMINATE（达标）
2. **用户终止** → 用户终止 → TERMINATE（终止），保存进度，说明恢复方法
3. **预算耗尽（达到最大轮次）** → 预算耗尽 → TERMINATE（预算耗尽），输出当前最优结果
4. **无法继续（连续 2 轮无任何维度进展）** → 无法继续 → TERMINATE（无法继续），上报原因，列出所需用户输入
...
所有质量门禁达标？
  ...
  └─ 否 →
      达到最大轮次？
        └─ 是 → 预算耗尽 → TERMINATE（预算耗尽，输出当前最优结果）
        └─ 否 →
            连续 2 轮所有维度均无进展（所有维度改善均 < 相对 3%）？
              └─ 是 → 无法继续 → TERMINATE（无法继续），输出当前最优结果并通知用户
                        （列出无法继续的原因和所需的用户输入，进入 AWAIT_USER）
              └─ 否 →
...
**达标终止**：
1. 更新 plan.md：状态 = 已完成，终止原因 = 达标终止
2. 生成最终报告（文件名见本文档"统一输出文件命名规则"表）
3. 清理临时文件（可选）

**预算耗尽**：
1. 更新 plan.md：状态 = 已完成，终止原因 = 预算耗尽
2. 生成"当前最优结果"报告
3. 明确标注哪些目标未达成

**用户终止**：
1. 立即保存当前进度
2. 更新 plan.md：状态 = 已完成，终止原因 = 用户终止
3. 生成中间报告
4. 说明恢复方法（下次运行如何从当前状态继续）

**无法继续**：
1. 更新 plan.md：状态 = 已完成，终止原因 = 无法继续
2. 清楚说明无法继续的原因
3. 列出需要用户提供的信息
4. 说明提供后如何继续
