...
|------|--------|--------|--------|--------|---------|
| T1 Research | 覆盖率 ≥ 85% | 可信度 ≥ 80% | 一致性 ≥ 90% | 完整性 ≥ 85% | 硬门禁（覆盖率/可信度）任一未达标；软门禁（一致性/完整性）记录到 findings.md + progress.md，继续迭代 |
| T2 Compare | 覆盖率 100% | 可信度 ≥ 80% | 偏见检查：每选项偏见分数 < 0.15（即10分量表差 < 1.5分）| 敏感性分析：key_assumptions 每项 ±20% 后推荐第1位不变 | 硬门禁（覆盖率/可信度/偏见检查）任一未达标；软门禁（敏感性分析）记录到 findings.md + progress.md，继续迭代 |
| T3 Iterate | KPI 达目标值（用户在 plan 中设定的 KPI 数值达到目标阈值）| — | — | — | KPI 未达标 |
| T4 Generate | 通过率 ≥ 95% | 平均分 ≥ 7/10 | — | — | 任一未达标 |
| T5 Deliver | 语法验证通过 | P1/P2 = 0 | Phase 4: service_list 全部 active（N/A 则跳过）+ health_check_url 返回200（N/A 则跳过）；两者至少须提供一项 | Phase 5: 用户输入 "用户确认（线上验收）"（人工线上验收确认） | 硬门禁（语法验证/P1P2/人工验收）任一未达标；软门禁（服务健康检查）记录到 progress.md，继续迭代 |
