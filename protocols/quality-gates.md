...
门禁分为硬门禁（必须通过）和软门禁（建议通过），不允许混合解释。

### 硬门禁（Hard Gate）

硬门禁失败 = 整轮不通过，必须在下轮修复后才能继续。
...
| T3 | KPI 达目标值 |
| T4 | 通过率 ≥ 95% |
| T5 | 语法验证通过、P1/P2 = 0、人工验收通过 |
| T6 | 安全性 ≥ 9/10、P1 = 0、安全 P2 = 0 |
| T7 | 架构 ≥ 8/10、性能 ≥ 8/10、稳定性 ≥ 8/10 |

### 软门禁（Soft Gate）

软门禁失败 = 记录到 progress.md + findings.md 但可继续迭代，不阻塞终止。
