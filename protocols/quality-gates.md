...
**终止层级**：
1. **质量门禁全部达标** → 达标终止（优先路径）
2. **用户终止** → 保存进度，可恢复
3. **预算耗尽（达到最大轮次）** → 输出当前最优结果，明确标注未达标项
4. **无法继续（无法继续推进）** → 上报用户，说明原因和所需信息

质量门禁是达标终止的判断标准；预算、用户终止、无法继续是其他终止路径，均在 `protocols/loop-protocol.md` 中有完整处理流程。
...
**通过标准**：阈值见下方门禁评估矩阵对应模板行（T1/T2/T4）。
...
对每个选项：
  ...
  通过条件：偏见分数 < 0.15（即 10 分量表上最大差异 < 1.5 分）（阈值定义见门禁评估矩阵 T2 行）
...
**通过标准**（阈值定义见门禁评估矩阵 T2 行）：所有选项的偏见分数 < 0.15（阈值定义见门禁评估矩阵 T2 行）（差异 ≥ 1.5 分须运行第三次独立评估，以多数结论为准）
...
**通过标准**（阈值定义见门禁评估矩阵 T2 行）：任意单一关键假设 ±20%（阈值定义见门禁评估矩阵 T2 行）变动后，推荐排名第 1 位不变
...
**要求**：plan 必须明确写出 KPI 名称、测量方法和目标阈值（例："API 响应时间 < 200ms"、"覆盖率 ≥ 90%"）。当 KPI 定义缺失时，在 OBSERVE 阶段暂停等待输入（进入 AWAIT_USER 状态），要求用户提供 KPI 定义、目标值和测量方法后继续。
...
**通用规则**：所有使用 grep/awk/文本匹配的验证步骤默认为 [L1] 近似检查。commands 和 templates 中的 grep 验证必须标注 `[L1]`。当 L1 结果不可信时，执行者应升级到 [L2]。
...
> 注意：安全严重级别分类以 `protocols/enterprise-standard.md` 为准。

| 严重级别 | 问题类型 |
|---------|---------|
| P1（必须修复）| SQL注入、命令注入、敏感数据暴露、XSS、路径穿越 |
| P2（必须修复）| 缺少输入验证 |
| P3（记录）| CORS 配置、过宽的错误消息 |

通过标准（分数阈值和计数阈值）见上方门禁评估矩阵及 T6 复合判定规则。

### 可靠性门禁（T5/T6）

| 严重级别 | 问题类型 |
|---------|---------|
| P1（必须修复）| 静默失败 (except: pass)、关键路径无异常处理 |
| P2（必须修复）| 无超时配置、缺少降级回退 |
| P3（记录）| 日志不完整、重试逻辑缺失 |
...
| T5 Deliver | 语法验证通过 | P1/P2 = 0 | Phase 4: git push 成功 + deploy_command 执行无报错（N/A 则跳过）+ service_list 全部 active（N/A 则跳过）+ health_check_url 返回200（N/A 则跳过）；完整 Phase 门禁见 protocols/delivery-phases.md | Phase 5: 用户输入 "verified"（人工线上验收确认） | 任一未达标 |
...
本章节是门禁评估矩阵 T6 行"复合判定"的完整展开。T6 达标条件为复合判定，以下两个条件**必须同时满足**：
...
> 本规则是 T6 达标的唯一判定标准。`protocols/delivery-phases.md` Phase 2 中 P2=0 是 T5 的交付标准，不适用于 T6。两者不冲突：T5 要求交付代码零 P2，T6 允许遗留少量低风险 P2 问题。
