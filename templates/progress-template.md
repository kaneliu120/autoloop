...
|---|------|--------|---------|------|
| 1 | {行动 1} | {agent} | {产出} | — |
| 2 | {行动 2} | {agent} | {产出} | 与3并行 |
| 3 | {行动 3} | {agent} | {产出} | 与2并行 |
...
后续轮次按照上述相同格式追加记录。每轮包含完整的 8 个阶段：观察（OBSERVE）、定向（ORIENT）、决策（DECIDE）、行动记录（ACT）、验证（VERIFY）、整合（SYNTHESIZE）、进化决策（EVOLVE）、反思（REFLECT）。
