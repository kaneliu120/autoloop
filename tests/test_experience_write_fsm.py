#!/usr/bin/env python3
"""`autoloop-experience cmd_write`: state machine and audit score chain."""
...
class TestWriteFsm(unittest.TestCase):
    ...
    def test_first_write_observation(self):
        m = self.mod
        ok = m.cmd_write(
            self.reg, "S-fsm-a", "Keep", "0.5", None,
            template="T1", dimension="d", tags=None,
        )
        self.assertTrue(ok)
        r = _row(m, self.reg, "S-fsm-a")
        self.assertEqual(r.get("status"), "Observation")
        self.assertEqual(r.get("use_count"), "1")

    def test_two_positive_deltas_promote_to_recommended(self):
        m = self.mod
        m.cmd_write(
            self.reg, "S-fsm-b", "Keep", "0.5", None,
            template="T1", dimension="d", tags=None,
        )
        m.cmd_write(
            self.reg, "S-fsm-b", "Pending Validation", "0.3", None,
            template="T1", dimension="d", tags=None,
        )
        r = _row(m, self.reg, "S-fsm-b")
        self.assertEqual(r.get("status"), "Recommended")
        self.assertEqual(r.get("use_count"), "2")

    def test_two_nonpositive_deltas_deprecate(self):
        m = self.mod
        m.cmd_write(
            self.reg, "S-fsm-c", "Pending Validation", "0.5", None,
            template="T1", dimension="d", tags=None,
        )
        m.cmd_write(
            self.reg, "S-fsm-c", "Avoid", "-0.2", None,
            template="T1", dimension="d", tags=None,
        )
        m.cmd_write(
            self.reg, "S-fsm-c", "Avoid", "-0.1", None,
            template="T1", dimension="d", tags=None,
        )
        r = _row(m, self.reg, "S-fsm-c")
        self.assertEqual(r.get("status"), "Deprecated")

    def test_deprecated_stays_deprecated_on_further_negative(self):
        m = self.mod
        sid = "S-fsm-d"
        m.cmd_write(self.reg, sid, "Keep", "1", None, template="T1", dimension="d", tags=None)
        m.cmd_write(self.reg, sid, "Avoid", "-0.5", None, template="T1", dimension="d", tags=None)
        m.cmd_write(self.reg, sid, "Avoid", "-0.5", None, template="T1", dimension="d", tags=None)
        self.assertEqual(_row(m, self.reg, sid).get("status"), "Deprecated")
        m.cmd_write(self.reg, sid, "Avoid", "-0.5", None, template="T1", dimension="d", tags=None)
        self.assertEqual(_row(m, self.reg, sid).get("status"), "Deprecated")
    ...
    def test_deprecated_recovers_on_positive_delta(self):
        ...
        m.cmd_write(self.reg, sid, "Avoid", "-0.5", None, template="T1", dimension="d", tags=None)
        self.assertEqual(_row(m, self.reg, sid).get("status"), "Deprecated")
        m.cmd_write(self.reg, sid, "Pending Validation", "0.5", None, template="T1", dimension="d", tags=None)
        self.assertEqual(_row(m, self.reg, sid).get("status"), "Observation")

    def test_audit_write_scores_chronological(self):
        m = self.mod
        ap = m._audit_path(self.reg)
        m.cmd_write(
            self.reg, "S-fsm-z", "Keep", "2.25", None,
            template="T1", dimension="d", tags=None,
        )
        self.assertEqual(m._audit_write_scores_chronological(ap, "S-fsm-z"), [2.25])
        m.cmd_write(
            self.reg, "S-fsm-z", "Keep", "0.25", None,
            template="T1", dimension="d", tags=None,
        )
    ...
    def test_avg_delta_is_mean_of_per_round_scores(self):
        ...
        for sc in ("1", "2", "4"):
            m.cmd_write(
                self.reg, sid, "Pending Validation", sc, None,
                template="T1", dimension="d", tags=None,
            )
    ...
    def test_four_writes_promote_to_candidate_default(self):
        ...
        r = _row(m, self.reg, sid)
        self.assertEqual(r.get("status"), "Candidate Default")
        self.assertEqual(r.get("use_count"), "4")

    def test_require_mechanism_blocks_write_when_use_count_ge_2(self):
        """`AUTOLOOP_EXPERIENCE_REQUIRE_MECHANISM=1`: writes that bring merged use_count to >=2 must include `mechanism`."""
        m = self.mod
        sid = "S-fsm-mech"
        m.cmd_write(
            self.reg, sid, "Keep", "0.5", None,
            template="T1", dimension="d", tags=None,
        )
        m.cmd_write(
            self.reg, sid, "Keep", "0.5", None,
    ...
    def test_explicit_status_skips_automatic_transitions(self):
        ...
        sid = "S-fsm-manual"
        m.cmd_write(
            self.reg, sid, "Avoid", "-1", None,
            status="Recommended", template="T1", dimension="d", tags=None,
        )
        m.cmd_write(
            self.reg, sid, "Avoid", "-1", None,
            status="Recommended", template="T1", dimension="d", tags=None,
